 ...
 * @author Compound
 */
contract Comptroller is ComptrollerV5Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError, Initializable {
    ...
    event CompSpeedUpdated(BToken indexed cToken, uint newSpeed);

    /// @notice Emitted when a new COMP speed is set for a contributor
    event ContributorCompSpeedUpdated(address indexed contributor, uint newSpeed);

    /// @notice Emitted when COMP is distributed to a supplier
    event DistributedSupplierComp(BToken indexed cToken, address indexed supplier, uint compDelta, uint compSupplyIndex);

    /// @notice Emitted when COMP is distributed to a borrower
    event DistributedBorrowerComp(BToken indexed cToken, address indexed borrower, uint compDelta, uint compBorrowIndex);
    ...
    function checkMembership(address account, BToken cToken) external view returns (bool) {
        return accountMembership[account][address(cToken)];
    }
    ...
    function enterMarket(address cToken, address borrower) public onlyPrimaryIndexToken returns(Error) {
        return addToMarketInternal(BToken(cToken), borrower);
    }
    ...
    function addToMarketInternal(BToken cToken, address borrower) internal returns (Error) {
        ...
        if (accountMembership[borrower][address(cToken)] == true) {
            // already joined
        ...
        accountMembership[borrower][address(cToken)] = true;
    ...
    function exitMarket(address cTokenAddress) external onlyPrimaryIndexToken override returns (uint) {
        ...
        if (!accountMembership[msg.sender][address(cToken)]) {
            return uint(Error.NO_ERROR);
        }

        /* Set cToken account membership to false */
        delete accountMembership[msg.sender][address(cToken)];
    ...
    function redeemAllowedInternal(address cToken, address redeemer, uint redeemTokens) internal view returns (uint) {
        ...
        if (!accountMembership[redeemer][address(cToken)]) {
            return uint(Error.NO_ERROR);
        }
    ...
    function borrowAllowed(address cToken, address borrower, uint borrowAmount) external override returns (uint) {
        ...
        if (!accountMembership[borrower][address(cToken)]) {
            ...
            assert(accountMembership[borrower][address(cToken)]);
        }
        ...
        (Error errAccLiquidity, , uint shortfall) = getHypotheticalAccountLiquidityInternal(borrower, BToken(cToken), 0, borrowAmount);
        if (errAccLiquidity != Error.NO_ERROR) {
            return uint(errAccLiquidity);
        }
        shortfall;
        // if (shortfall > 0) {
        //     return uint(Error.INSUFFICIENT_LIQUIDITY);
        ...
        // Keep the flywheel moving
        Exp memory borrowIndex = Exp({mantissa: BToken(cToken).borrowIndex()});
    ...
    function _setPrimaryIndexTokenAddress(address _newPrimaryIndexToken) external returns(uint){
         // Check caller is admin
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_LIQUIDATION_INCENTIVE_OWNER_CHECK);
        }
    ...
    function _supportMarket(BToken cToken) external returns (uint) {
        ...
        cToken.isCToken(); // Sanity check to make sure its really a BToken
     ...
     */
    function setCompSpeedInternal(BToken cToken, uint compSpeed) internal {
    ...
    function updateCompSupplyIndex(address cToken) internal {
        ...
        if (deltaBlocks > 0 && supplySpeed > 0) {
            uint supplyTokens = BToken(cToken).totalSupply();
    ...
    function updateCompBorrowIndex(address cToken, Exp memory marketBorrowIndex) internal {
        ...
        if (deltaBlocks > 0 && borrowSpeed > 0) {
            uint borrowAmount = div_(BToken(cToken).totalBorrows(), marketBorrowIndex);
    ...
    function distributeSupplierComp(address cToken, address supplier) internal {
        ...
        uint supplierTokens = BToken(cToken).balanceOf(supplier);
        uint supplierDelta = mul_(supplierTokens, deltaIndex);
        uint supplierAccrued = add_(compAccrued[supplier], supplierDelta);
        compAccrued[supplier] = supplierAccrued;
        emit DistributedSupplierComp(BToken(cToken), supplier, supplierDelta, supplyIndex.mantissa);
    }
    ...
    function distributeBorrowerComp(address cToken, address borrower, Exp memory marketBorrowIndex) internal {
        ...
        if (borrowerIndex.mantissa > 0) {
            ...
            uint borrowerAmount = div_(BToken(cToken).borrowBalanceStored(borrower), marketBorrowIndex);
            uint borrowerDelta = mul_(borrowerAmount, deltaIndex);
            uint borrowerAccrued = add_(compAccrued[borrower], borrowerDelta);
            compAccrued[borrower] = borrowerAccrued;
            emit DistributedBorrowerComp(BToken(cToken), borrower, borrowerDelta, borrowIndex.mantissa);
        }
    }
     ...
     */
    function claimComp(address holder, BToken[] memory cTokens) public {
     ...
     */
    function claimComp(address[] memory holders, BToken[] memory cTokens, bool borrowers, bool suppliers) public {
        for (uint i = 0; i < cTokens.length; i++) {
            BToken cToken = cTokens[i];
     ...
     * @param compSpeed New COMP speed for market
     */
    function _setCompSpeed(BToken cToken, uint compSpeed) public {
