...
The configuration information is stored in the `scripts/V2/config/{network}_{chain}` folder for ethereum networks and in the `deploy/V2/config_{chain}` folder for zksync network. This folder contains subfolders that correspond to the networks used for testing and deploy.
...
```
//file: ./scripts/V2/config/arbitrum_goerli/config_gerenal.json
...
- In the `scripts/V2/config/arbitrum_goerli` folder, copy the entire content from the `config.example.json` file to a file named `config.json`.
...
In the folders `scripts/V2/config/hardhat_*/` there are files `config.testing.json`, these files are used to configure parameters when testing, **_Please do not change these files_**.
