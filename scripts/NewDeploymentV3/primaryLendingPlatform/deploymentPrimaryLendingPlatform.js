...
const configGeneralFile = path.join(__dirname, `../../config/V3/${network}/config_general.json`);
const configGeneral = require(configGeneralFile);
const configFile = path.join(__dirname, `../../config/V3/${network}/config.json`);
const config = require(configFile);
const verifyFilePath = path.join(__dirname, `../../config/V3/${network}/verify.json`);
...
module.exports = {
    ...
    deploymentPrimaryLendingPlatform: async function () {
        ...
        let PrimaryIndexToken = await hre.ethers.getContractFactory("PrimaryIndexTokenV3");
        let PrimaryIndexTokenAtomicRepayment = await hre.ethers.getContractFactory("PrimaryIndexTokenAtomicRepaymentV3");
        let PrimaryIndexTokenLiquidation = await hre.ethers.getContractFactory("PrimaryIndexTokenLiquidationV3");
        let PrimaryIndexTokenLeverage = await hre.ethers.getContractFactory("PrimaryIndexTokenLeverageV3");
        let PrimaryIndexTokenModerator = await hre.ethers.getContractFactory("PrimaryIndexTokenModeratorV3");
        let PrimaryIndexTokenWrappedTokenGateway = await hre.ethers.getContractFactory("PrimaryIndexTokenWrappedTokenGatewayV3");
        ...
        console.log("PrimaryIndexTokenAtomicRepayment proxy address: " + primaryIndexTokenAtomicRepaymentProxyAddress);
        ...
        if (!isModeratorLeverage) {
            await pitLeverage.initialize(primaryIndexTokenProxyAddress, primaryIndexTokenAtomicRepaymentProxyAddress, augustusParaswap, AUGUSTUS_REGISTRY)
                .then(function () {
