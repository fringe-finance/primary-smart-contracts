...
```solidity
function depositFor(address account, uint256 amount) external nonpayable returns (bool)
```
...
```solidity
function underlying() external view returns (contract IERC20Upgradeable)
```
...
```solidity
function withdrawTo(address account, uint256 amount) external nonpayable returns (bool)
```
