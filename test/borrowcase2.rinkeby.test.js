...
log4js.configure({
    appenders: {
        primaryIndexToken: { 
            type: "file",
            filename: "test/logs/borrowcase2Rinkeby.log"
        }
    },
    categories: {
        default: { 
...
contract('PrimaryIndexToken', (accounts) => {
    ...
    it('Supplier1 supply USDCTest', async()=>{
        ...
        let amountLendingToken = USDCmultiplier.mul(new BN(2_000_000));
    ...
    it('Borrower1 borrow USDC with collateral PRJ1',async()=>{
        ...
        let borrower1PITbalanceForPositionBeforeBefore = await getBalanceOfPitPosition(borrower1,prjId);
        await printBalanceOfPitForPosition(borrower1PITbalanceForPositionBeforeBefore);
        ...
        let amountLendingTokenToBorrow = borrower1PITbalanceForPositionBeforeBefore.div(toBN(4));
        ...
        let borrower1PITbalanceForPositionBeforeAfter = await getBalanceOfPitPosition(borrower1,prjId);
        await printBalanceOfPitForPosition(borrower1PITbalanceForPositionBeforeAfter);
    ...
    it('Borrower1 extra borrow USDC with collateral PRJ1',async()=>{
        pitLogger.info();
        pitLogger.info("*******Borrower1 extra borrow USDC with collateral PRJ1 Test******* ");
    ...
    it('Borrower1 extra borrow USDC with collateral PRJ1',async()=>{
        ...
        let amountLendingTokenToBorrow = borrower1PITbalanceForPositionBeforeBefore.div(toBN(2));
        pitLogger.info("Borrowed "+ amountLendingTokenToBorrow+" tokens");
        let borrowRes = await primaryIndexToken.borrow(lendingTokenId,amountLendingTokenToBorrow,PRJaddress,toBN(10),{from:borrower1});
        for(var log of borrowRes['logs']){
            if(log['event']=="Test2"){
    ...
    it('Borrower1 1/2 borrow USDC with collateral PRJ2',async()=>{
        pitLogger.info();
        pitLogger.info("*******Borrower1 1/2 borrow USDC with collateral PRJ2 Test******* ");
    ...
    it('Borrower1 1/4 extra borrow USDC with collateral PRJ2',async()=>{
        pitLogger.info();
        pitLogger.info("*******Borrower1 1/4 extra borrow USDC with collateral PRJ2 Test******* ");
    ...
    it('Borrower1 1/5 extra borrow USDC with collateral PRJ3',async()=>{
        pitLogger.info();
        pitLogger.info("*******Borrower1 1/5 extra borrow USDC with collateral PRJ3 Test******* ");
        ...
        let amountLendingTokenToBorrow = borrower1PITbalanceForPositionBeforeBefore.div(toBN(5));
        pitLogger.info("Borrowed "+ amountLendingTokenToBorrow+" tokens");
        let borrowRes = await primaryIndexToken.borrow(lendingTokenId,amountLendingTokenToBorrow,PRJaddress,toBN(10),{from:borrower1});
        for(var log of borrowRes['logs']){
            if(log['event']=="Test2"){
    ...
    it('Borrower1 1/2 extra borrow USDC with collateral PRJ3',async()=>{
        pitLogger.info();
        pitLogger.info("*******Borrower1 1/2 extra borrow USDC with collateral PRJ3 Test******* ");
