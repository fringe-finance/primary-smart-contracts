...
const { deployment } = require("../../../scripts/V2/deployPLP/deploymentPLP");
const { ethers } = require("ethers");
const { expect } = require("chai");
const helpers = require("@nomicfoundation/hardhat-network-helpers");
let { deploymentMockToken } = require("../../../scripts/V2/deployPLP/deploymentMockToken");
const { getPriceFeedsUpdateData } = require("./utils/utilities");

const fs = require("fs");
const path = require("path");
const OOEJson = require("./artifacts/OpenOceanExchange.json");
const configGeneralFile = path.join(__dirname, `../../../scripts/V2/config/hardhat_zksync_on_polygon_mainnet/config_general.json`);
const configGeneral = require(configGeneralFile);
const configTestingFile = path.join(__dirname, `../../../scripts/V2/config/hardhat_zksync_on_polygon_mainnet/config_testing.json`);
...
describe("PrimaryLendingPlatformWrappedTokenGateway", function () {
    ...
    async function loadFixture() {
        ...
        }
        {
            ...
            let PLPAtomicRepay = await hre.ethers.getContractFactory("PrimaryLendingPlatformAtomicRepaymentV2Zksync");
            let PLPLeverage = await hre.ethers.getContractFactory("PrimaryLendingPlatformLeverageZksync");
            let PLPLiquidation = await hre.ethers.getContractFactory("PrimaryLendingPlatformLiquidationV2Zksync");
            let PLPWTG = await hre.ethers.getContractFactory("PrimaryLendingPlatformWrappedTokenGatewayV2Zksync");
            let PLPModerator = await hre.ethers.getContractFactory("PrimaryLendingPlatformModeratorV2");
    ...
    describe("withdraw", function () {
        ...
        it("10. Failure: Should revert when priceIds.length != updateData.length", async function () {
    ...
    describe("borrow", function () {
        ...
        it("13. Failure: Should revert when priceIds.length != updateData.length", async function () {
            let projectToken = prj1.address;
            let lendingToken = weth.address;
            let lendingTokenAmount = toBN(1);
    ...
    describe("leveragedBorrowWithProjectETH", function () {
        ...
        it("23. Failure: Should revert when priceIds.length != updateData.length", async function () {
            let projectToken = weth.address;
            let lendingToken = usdc.address;
