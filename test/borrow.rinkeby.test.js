...
contract('PrimaryIndexToken', (accounts) => {
    ...
    it('Borrower1 borrow USDC with collateral PRJ1',async()=>{
        ...
        let borrower1CPITbalanceBefore = await getERC20balance(bPrimaryIndexTokenAddress,borrower1);
        ...
        let amountLendingTokenToBorrow = borrower1PITbalanceBefore;//USDCmultiplier.mul(new BN(200_000));
        ...
        let borrower1CPITbalanceAfter = await getERC20balance(bPrimaryIndexTokenAddress,borrower1);
    ...
    it('Borrower1 repay all USDCTest borrow for PRJ4 position',async()=>{
        pitLogger.info();
        pitLogger.info("*******Borrower1 repay all USDCTest borrow for PRJ4 position Test******* ");
        ...
        let amountLendingTokenToRepay = toBN(2).pow(toBN(256)).sub(toBN(2));//borrower1BorrowPositionBefore['0'].div(toBN(2));
        await approveTransferFrom(USDCaddress,bUSDCAddress,borrower1,amountLendingTokenToRepay);
        let repayRes = await primaryIndexToken.repayBorrow(lendingTokenId,amountLendingTokenToRepay,PRJaddress,toBN(10),{from:borrower1});
        for(var log of repayRes['logs']){
            if(log['event']=="Test4"){
